...
uint16_t servo3_CCW_limit = 500;   // Límite superior
...
// =========================
// Estructura para movimientos simultáneos
// =========================
struct SimulCommand {
  bool valid;
  int servoID;
  int position;      // Valor ya convertido a unidad interna
  String type;       // "unit" o "angle" (para referencia)
  // Para servos 1 y 2:
  int p;
  int i;
  int d;
...
// Usaremos un arreglo índice 1..3 (dejando el índice 0 sin uso)
SimulCommand simulCommands[4];
...
void setup() {
  ...
  // Inicializar los comandos simultáneos
  for (int i = 1; i < 4; i++) {
...
void loop() {
  ...
  if (client) {
    ...
    // ===============================
    // Endpoint para guardar movimiento simultáneo
    // ===============================
    if (request.indexOf("/guardar_simul") != -1) {
      ...
      if (servoID == SERVO3_ID) {
        // Para el servo 3: si se ingresa en ángulo se convierte
        if (typeStr == "angle") {
          posValue = map(posValue, 0, MAX_ANGLE_1, 0, MAX_UNIT_1);
        }
        posValue = constrain(posValue, 0, MAX_UNIT_1);
        simulCommands[servoID].position = posValue;
        simulCommands[servoID].type = typeStr;
      } else {
        // Para servos 1 y 2, además de la posición se guardan PID, velocidad y aceleración
        ...
        if (typeStr == "angle") {
          // Convertir de ángulo (0-360) a unidades internas (0-4095)
          posValue = map(posValue, 0, MAX_ANGLE_2, 0, MAX_UNIT_2);
        }
        posValue = constrain(posValue, 0, MAX_UNIT_2);
        simulCommands[servoID].position = posValue;
        simulCommands[servoID].type = typeStr;
        simulCommands[servoID].p = pVal;
        simulCommands[servoID].i = iVal;
        simulCommands[servoID].d = dVal;
        simulCommands[servoID].velocity = vel;
        simulCommands[servoID].acceleration = acc;
      }
      simulCommands[servoID].servoID = servoID;
      simulCommands[servoID].valid = true;
      
      // Respuesta informando que se ha guardado el movimiento
      client.println("HTTP/1.1 200 OK");
      client.println("Content-Type: text/html");
      client.println();
      client.println("<html><head><meta charset='UTF-8'><title>Movimiento Guardado</title></head><body>");
      client.println("<h1>Movimiento simultáneo guardado para el Servo " + String(servoID) + ".</h1>");
    ...
    // ===============================
    // Endpoint para confirmar movimientos simultáneos
    // ===============================
    if (request.indexOf("/confirm_simul") != -1) {
      ...
      for (int id = 1; id <= 2; id++) {
        if (simulCommands[id].valid) {
          ...
          dxl.setGoalPosition(id, simulCommands[id].position);
          simulCommands[id].valid = false;
          Serial.println("Servo " + String(id) + " ejecuta movimiento simultáneo a posición: " + String(simulCommands[id].position));
        }
      }
      // Para servo 3 (Protocolo 1.0)
      if (simulCommands[SERVO3_ID].valid) {
        dxl.setPortProtocolVersion(PROTOCOL_VERSION_1);
        uint16_t pos = simulCommands[SERVO3_ID].position;
        if (dxl.write(SERVO3_ID, GOAL_POSITION_ADDR, (uint8_t*)&pos, GOAL_POSITION_ADDR_LEN, TIMEOUT)) {
          Serial.println("Servo 3 ejecuta movimiento simultáneo a posición: " + String(pos));
        } else {
          Serial.println("Error moviendo el servo 3 en movimiento simultáneo.");
        }
        simulCommands[SERVO3_ID].valid = false;
        // Regresar a protocolo 2.0 para los demás
        dxl.setPortProtocolVersion(PROTOCOL_VERSION_2);
      }
      
      // Respuesta informando que se ejecutaron los movimientos
      client.println("HTTP/1.1 200 OK");
      client.println("Content-Type: text/html");
      client.println();
      client.println("<html><head><meta charset='UTF-8'><title>Movimientos Ejecutados</title></head><body>");
      client.println("<h1>Movimientos simultáneos ejecutados.</h1>");
      client.println("<a href='/'>Volver</a>");
      client.println("</body></html>");
      client.stop();
      return;
    ...
    // Procesa el ajuste de posición individual (ya existente)
    ...
    client.println("    form { margin-bottom: 20px; padding: 10px; border: 1px solid #aaa; }");
    client.println("  </style>");
    client.println("</head>");
    client.println("<body>");
    ...
    client.println("    <h3>Control de Posición (Ejecución inmediata)</h3>");
    ...
    client.println("    </form>");
    client.println("    <h3>Guardar Movimiento Simultáneo</h3>");
    ...
    client.println("      <input type=\"radio\" name=\"type\" value=\"unit\" checked> Unidad interna<br>");
    client.println("      <input type=\"radio\" name=\"type\" value=\"angle\"> Ángulo (0-360)<br>");
    client.println("      <label>P:</label><input type=\"number\" name=\"p\" min=\"0\" max=\"32767\" value=\"" + String(position_p_gain[0]) + "\" required><br>");
    client.println("      <label>I:</label><input type=\"number\" name=\"i\" min=\"0\" max=\"32767\" value=\"" + String(position_i_gain[0]) + "\" required><br>");
    client.println("      <label>D:</label><input type=\"number\" name=\"d\" min=\"0\" max=\"32767\" value=\"" + String(position_d_gain[0]) + "\" required><br>");
    client.println("      <label>Velocidad:</label><input type=\"number\" name=\"v\" min=\"0\" max=\"1023\" value=\"" + String(velocity[0]) + "\" required><br>");
    client.println("      <label>Aceleración:</label><input type=\"number\" name=\"a\" min=\"0\" max=\"1023\" value=\"" + String(acceleration[0]) + "\" required><br>");
    client.println("      <button type=\"submit\">Guardar Movimiento Simultáneo</button>");
    client.println("    </form>");
    client.println("    <h3>Ajustar PID</h3>");
    client.println("    <form action=\"/set_pid\" method=\"get\">");
    client.println("      <input type=\"hidden\" name=\"servo\" value=\"1\">");
    client.println("      <input type=\"hidden\" name=\"tab\" value=\"pestana1\">");
    client.println("      <label>P:</label><input type=\"number\" name=\"p\" min=\"0\" max=\"32767\" value=\"" + String(dxl.readControlTableItem(POSITION_P_GAIN, 1)) + "\" required><br>");
    client.println("      <label>I:</label><input type=\"number\" name=\"i\" min=\"0\" max=\"32767\" value=\"" + String(dxl.readControlTableItem(POSITION_I_GAIN, 1)) + "\" required><br>");
    client.println("      <label>D:</label><input type=\"number\" name=\"d\" min=\"0\" max=\"32767\" value=\"" + String(dxl.readControlTableItem(POSITION_D_GAIN, 1)) + "\" required><br>");
    ...
    client.println("      <input type=\"hidden\" name=\"servo\" value=\"1\">");
    client.println("      <input type=\"hidden\" name=\"tab\" value=\"pestana1\">");
    client.println("      <label>Velocidad:</label><input type=\"number\" name=\"v\" min=\"0\" max=\"1023\" value=\"" + String(dxl.readControlTableItem(PROFILE_VELOCITY, 1)) + "\" required><br>");
    ...
    client.println("      <input type=\"hidden\" name=\"servo\" value=\"1\">");
    client.println("      <input type=\"hidden\" name=\"tab\" value=\"pestana1\">");
    client.println("      <label>Aceleración:</label><input type=\"number\" name=\"a\" min=\"0\" max=\"1023\" value=\"" + String(dxl.readControlTableItem(PROFILE_ACCELERATION, 1)) + "\" required><br>");
    ...
    client.println("      <input type=\"radio\" name=\"type\" value=\"unit\" checked> Unidad interna<br>");
    client.println("      <input type=\"radio\" name=\"type\" value=\"angle\"> Ángulo (0-360)<br>");
    client.println("      <label>P:</label><input type=\"number\" name=\"p\" min=\"0\" max=\"32767\" value=\"" + String(dxl.readControlTableItem(POSITION_P_GAIN, 2)) + "\" required><br>");
    client.println("      <label>I:</label><input type=\"number\" name=\"i\" min=\"0\" max=\"32767\" value=\"" + String(dxl.readControlTableItem(POSITION_I_GAIN, 2)) + "\" required><br>");
    client.println("      <label>D:</label><input type=\"number\" name=\"d\" min=\"0\" max=\"32767\" value=\"" + String(dxl.readControlTableItem(POSITION_D_GAIN, 2)) + "\" required><br>");
    client.println("      <label>Velocidad:</label><input type=\"number\" name=\"v\" min=\"0\" max=\"1023\" value=\"" + String(dxl.readControlTableItem(PROFILE_VELOCITY, 2)) + "\" required><br>");
    client.println("      <label>Aceleración:</label><input type=\"number\" name=\"a\" min=\"0\" max=\"1023\" value=\"" + String(dxl.readControlTableItem(PROFILE_ACCELERATION, 2)) + "\" required><br>");
    client.println("      <button type=\"submit\">Guardar Movimiento Simultáneo</button>");
    client.println("    </form>");
    client.println("    <h3>Ajustar PID</h3>");
    client.println("    <form action=\"/set_pid\" method=\"get\">");
    ...
    // Botón global para confirmar todos los movimientos simultáneos
    client.println("  <div style='margin:20px; padding:10px; border:1px solid #555;'>");
    client.println("    <h3>Confirmar Movimientos Simultáneos</h3>");
    client.println("    <form action=\"/confirm_simul\" method=\"get\">");
    client.println("      <button type=\"submit\">Confirmar Todos los Movimientos</button>");
    client.println("    </form>");
    client.println("  </div>");
